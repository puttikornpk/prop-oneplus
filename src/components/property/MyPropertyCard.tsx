...
export const MyPropertyCard: React.FC<MyPropertyCardProps> = ({ property }) => {
    ...
    const formatPrice = (price?: number | null) => {
        return price ? `฿${price.toLocaleString()}` : 'ไม่ระบุราคา';
    };
            ...
            <div className="flex-1 p-4 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-start mb-1">
                        <div className='flex gap-2 items-center text-xs text-slate-500 mb-1'>
                            <MapPin size={12} />
                            <span>{property.address || 'ไม่ระบุที่อยู่'}</span>
                        </div>
                        <button
                    ...
                    <h3 className="font-bold text-lg text-slate-800 line-clamp-1 mb-1">{property.topic || 'ไม่มีหัวข้อ'}</h3>

                    <div className="text-slate-900 font-bold text-xl">
                        {formatPrice(property.price)}
                    </div>
                    <p className="text-xs text-slate-500 mt-1">
                        {property.landRai ? `${property.landRai} ไร่ ` : ''}
                        {property.landNgan ? `${property.landNgan} งาน ` : ''}
                        {property.landSqWah ? `${property.landSqWah} ตร.ว.` : ''}
                    </p>
                </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-50 relative">
                    <div className="text-xs text-slate-400">
                        ร่างเมื่อ : {property.createdAt ? formatDate(property.createdAt) : '-'}
                    </div>
                                    ...
                                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
                                        <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm pointer-events-auto overflow-hidden animate-in zoom-in-95 fade-in duration-200">
                                            ...
                                            <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                                <h3 className="font-bold text-slate-800">จัดการประกาศ</h3>
                                                <button
                                                    onClick={() => setIsMenuOpen(false)}
                                            ...
                                            <div className="p-2">
                                                ...
                                                >
                                                    ...
                                                    <div className="text-left">
                                                        <div className="font-semibold">แก้ไข</div>
                                                        <div className="text-xs text-slate-400">แก้ไขรายละเอียดประกาศ</div>
                                                    </div>
                                                </button>
                                                <button
                                                    onClick={() => {
                                                ...
                                                >
                                                    ...
                                                    <div className="text-left">
                                                        <div className="font-semibold">โน้ต</div>
                                                        <div className="text-xs text-slate-400">เพิ่มบันทึกช่วยจำ</div>
                                                    </div>
                                                </button>
                                                ...
                                                >
                                                    ...
                                                    <div className="text-left">
                                                        <div className="font-semibold">ลบประกาศ</div>
                                                        <div className="text-xs text-red-400">ลบประกาศนี้ออกจากระบบ</div>
                                                    </div>
                                                </button>
                                            </div>
                                        </div>
