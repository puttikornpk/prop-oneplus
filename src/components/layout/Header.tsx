...
export const Header = () => {
    ...
    return (
        <header className="sticky top-0 z-50 bg-gradient-to-r from-brand-200 via-white to-brand-200 backdrop-blur-md shadow-sm border-b border-brand-100">
            ...
            <div className="hidden md:flex justify-end items-center h-10 px-4 container mx-auto gap-4">
                ...
                <button className="p-2 text-slate-600 hover:bg-white/50 rounded-full transition-all duration-300 hover:scale-110 active:scale-90 relative hover:text-brand-600">
                    <Bell size={18} />
                    <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse border border-white"></span>
                </button>
                    ...
                    {user ? (
                        <div className="relative" ref={userMenuRef}>
                            <button
                                ...
                                className="p-1 text-slate-600 hover:bg-white/50 rounded-full transition-all duration-300 hover:scale-110 active:scale-90 hover:text-brand-600 flex items-center gap-2"
                            >
            ...
            <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                ...
                {/* User Actions - Mobile Only + List Property Button */}
                <div className="flex items-center gap-4">
                    <button
                        onClick={handleListProperty}
                        className="hidden sm:flex bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-full font-medium text-sm transition-all shadow-lg shadow-red-200 hover:shadow-red-300 hover:scale-105 active:scale-95 duration-300"
                    >
                        {t('listProperty')}
                    </button>

                    {/* Mobile Only: Menu Button and condensed profile access via Menu or just button */}
                    <button
                        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                        className="md:hidden p-2 text-slate-600 hover:bg-white/50 rounded-full transition-all duration-300 hover:scale-110 active:scale-90 hover:text-brand-600"
                    >
                        {mobileMenuOpen ? <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg> : <Menu size={20} />}
                    </button>
                </div>
            </div>
            ...
            {mobileMenuOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-brand-100 shadow-lg animate-in slide-in-from-top-2 duration-200">
                    <div className="container mx-auto px-4 py-4 flex flex-col gap-2">
                        ...
                        <Link href="/" className="p-3 text-slate-700 hover:bg-brand-50 hover:text-brand-600 rounded-lg transition-colors font-medium flex items-center gap-3">
                            {t('home')}
                        </Link>
                        <Link href="#" className="p-3 text-slate-700 hover:bg-brand-50 hover:text-brand-600 rounded-lg transition-colors font-medium flex items-center gap-3">
                            {t('recSearch')}
                        ...
                        <button onClick={handleListProperty} className="w-full text-left p-3 text-red-600 hover:bg-red-50 rounded-lg transition-colors font-medium flex items-center gap-3">
                            {t('listProperty')}
                        </button>
                        ...
                        {!user && (
                            <div className="grid grid-cols-2 gap-3 mt-2">
                                <button onClick={() => setLoginOpen(true)} className="py-2.5 px-4 rounded-lg border border-brand-200 text-brand-600 font-medium hover:bg-brand-50">{t('login')}</button>
                                <button onClick={() => setRegisterOpen(true)} className="py-2.5 px-4 rounded-lg bg-brand-600 text-white font-medium hover:bg-brand-700">{t('freeRegister')}</button>
                            </div>
                        )}
